...
# Declaring all the functions# Functions
for taking inputs
def inputFrom():
  mText = fromCity.get()# mlabel1 = Label(tableFrame3, text = mText).grid()
def inputTo():
  mText = toCity.get()# mlabel1 = Label(tableFrame3, text = mText).grid()
def inputDepDate():
  mText = departDate.get()# mlabel1 = Label(tableFrame3, text = mText).grid()

def inputDepMon():
  mText = departMonth.get()# mlabel1 = Label(tableFrame3, text = mText).grid()
def mAbout():
...
conarray = [i + "\n" + j + "\n" + k
  for i, j, k in zip(tim[::3], tim[1::3], tim[2::3])
]
...
conarray2 = [i + " " + j
  for i, j in zip(airname, flightnumgo)
]
...
## Checking and storing price
for flights in Paytm into the array
...
## Creating Frames
for input, Sorting buttons and output
